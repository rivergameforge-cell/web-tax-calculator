# 한국 세금 계산기 — Codex 작업 가이드

## 프로젝트 개요
한국 주요 세금을 계산할 수 있는 **Vanilla HTML/CSS/JS SPA**입니다.
빌드 도구 없이 `python3 -m http.server 8080`으로 바로 실행됩니다.

---

## 기술 스택 & 제약사항

| 항목 | 내용 |
|------|------|
| 언어 | HTML5 / CSS3 / JavaScript ES6+ |
| 프레임워크 | **없음** (Vanilla만 사용) |
| 빌드 도구 | **없음** — `<script src="...?v=N">` 캐시버스팅만 사용 |
| 폰트 | Pretendard CDN + 시스템 폰트 fallback |
| 로컬 서버 | `python3 -m http.server 8080` |

---

## 디렉터리 구조

```
web_tex_calculator/
├── index.html                          # 단일 SPA 진입점, 모든 뷰 포함
├── css/
│   ├── reset.css                       # 브라우저 reset
│   ├── variables.css                   # CSS Custom Properties (라이트/다크)
│   ├── layout.css                      # 앱 셸, 헤더, 사이드바, 탭바
│   ├── components.css                  # 카드, 폼, 인풋, 버튼, 배지
│   ├── calculator.css                  # 계산기 전용 스타일
│   └── responsive.css                  # 반응형 브레이크포인트
├── js/
│   ├── theme.js                        # 라이트/다크 모드 토글
│   ├── ui.js                           # fmtWon, toast, debounce 등 유틸
│   ├── router.js                       # 해시 기반 라우터
│   ├── ads.js                          # 전면광고(Page Visibility), 앵커광고
│   ├── app.js                          # NAV_CONFIG, TAB_CONFIG, 사이드바 빌드
│   └── calculators/
│       ├── loan/
│       │   └── loan.js
│       ├── real-estate/
│       │   ├── acquisition.js
│       │   ├── capital-gains.js
│       │   └── commission.js
│       ├── vehicle/
│       │   └── vehicle-tax.js
│       └── stocks/
│           ├── foreign.js
│           └── transaction.js
└── .Codex/
    └── launch.json                     # 로컬 서버 자동 실행 설정
```

---

## 새 계산기 추가 시 체크리스트

> **⚠️ 중요: 모든 변경 파일을 한 커밋에 포함할 것!**
> `js/app.js` (NAV_CONFIG/TAB_CONFIG), 계산기 JS 파일, `index.html` 변경을 **반드시 같은 커밋**에 넣어야 한다.
> 파일을 나눠서 커밋하면 배포 시 사이드바/탭에 항목이 안 나오는 버그 발생.
> 커밋 전 `git status`로 수정된 파일이 모두 staged 되었는지 반드시 확인!

> **⚠️ 중요: 작업 완료 후 반드시 커밋 & 푸시할 것!**
> 사용자는 배포된 GitHub Pages 사이트(`https://taxcalc.co.kr/`)에서 테스트한다.
> 로컬 파일만 수정하고 커밋/푸시하지 않으면 사용자 화면에는 변경사항이 **전혀 반영되지 않는다**.
> 파일 수정 → `git add` → `git commit` → `git push`까지 **한 세트**로 묶어 진행할 것.
> "수정했다"고 답하기 전에 `git status`로 working tree가 clean한지, `git log`로 푸시된 커밋이 최신인지 확인 필수!

1. **`js/app.js`** — `NAV_CONFIG`와 `TAB_CONFIG`에 항목 추가
2. **`js/calculators/<category>/<name>.js`** — 계산기 모듈 생성
   - 파일 맨 아래 `const Calc<Name> = (() => { ... return { init }; })();` 패턴 사용
3. **`index.html`** — 세 곳 수정
   - `<div id="view-<category>-<name>" class="calculator-view">` 뷰 HTML 추가
   - `<script src="js/calculators/<category>/<name>.js?v=N">` 스크립트 태그 추가
   - `DOMContentLoaded` 블록에 `Calc<Name>.init();` 추가
4. **캐시버스팅** — 스크립트 수정 시 `?v=N` 숫자 올리기
5. **커밋 전 검증** — `git diff --name-only`로 위 1~4의 파일이 모두 포함되었는지 확인

---

## 블로그 글 작성 요청 시 고정 순서

사용자가 "글 써줘", "포스팅 만들어줘"처럼 세금 계산기 콘텐츠 작성을 요청하면
항상 아래 순서대로 진행한다.

> **필수 스킬:** 블로그 주제 추천, 작성, 수정, 검수 요청에는 `$korean-tax-blog-writer` 스킬을 사용한다.
> 스킬의 정확성 우선 원칙과 플랫폼별 역할을 이 문서의 작업 순서·파일 형식 규칙과 함께 적용한다.

### 플랫폼별 글쓴이 역할

- **세금계산기 사이트:** 20년차 경제 교수의 관점으로 정확하고 객관적인 사실, 계산 구조, 제도적 맥락을 정보 전달 중심으로 설명한다.
- **티스토리:** 30년차 경제 전문 세무사의 관점으로 신고 실무와 독자 행동을 정확히 안내하고, 사실에 근거한 후킹·광고 문구·CTA를 능숙하게 활용한다.
- **네이버:** 20년차 경제 전문 블로거의 관점으로 정확한 정보를 친절하게 풀고, 독자를 불편하게 하지 않는 가벼운 상황형 유머를 적절히 사용한다.
- 위 역할은 글의 깊이와 문체를 정하는 편집 기준이다. 실제 교수·세무사 경력을 사칭하거나 근거 없는 절세·환급 결과를 보장하지 않는다.

> **중요: 플랫폼별 글과 썸네일은 반드시 차별화할 것!**
> 같은 주제를 다루더라도 세금계산기 사이트용, 티스토리용, 네이버용은
> 제목 롱테일, 도입부, 문체, 예시 구성, CTA 방식, 썸네일 콘셉트가 모두 달라야 한다.
> 한 플랫폼 글을 다른 플랫폼에 문장만 조금 바꿔 재사용하지 않는다.
> 독자가 봤을 때 글쓴이와 편집 의도가 다르게 느껴질 정도로 분리한다.

1. **세금 계산기용 포스팅 작성**
   - `blog_post/*.md` 원고를 작성한다.
   - 세금 계산기 사이트에 맞는 SEO 제목, 설명, FAQ, CTA를 포함한다.
   - 계산기 링크는 `https://taxcalc.co.kr/#<category>/<item>` 형식으로 연결한다.
   - 문체는 검색엔진용 정석 SEO 설명글로 작성한다.
   - 계산 공식, 기준, 표준 예시, FAQ, 계산기 CTA를 중심으로 구성한다.
   - 사이트용 롱테일 제목은 "계산", "기준", "예시", "얼마"처럼 검색 의도가 명확한 키워드를 포함한다.
2. **세금 계산기용 포스팅 배포**
   - 정적 블로그 HTML을 생성하고 `blog/index.html`, `sitemap.xml` 등 필요한 파일을 갱신한다.
   - 사이트용 썸네일은 `1200x630` WebP로 생성한다.
   - 사이트용 썸네일은 OG/검색 공유에 맞게 신뢰감 있는 정보형 콘셉트로 만든다.
   - 로컬 검증 후 커밋하고 `main`에 푸시한다.
3. **티스토리용 포스팅 작성**
   - `blog_post/*_티스토리.txt` 파일을 별도로 작성한다.
   - 티스토리 HTML 모드에 바로 붙여넣을 수 있는 `<p data-ke-size="size16">` 형식을 사용한다.
   - 본문 상단부와 중간/하단부에 애드센스 광고 블록을 넣는다.
   - CTA 버튼으로 `taxcalc.co.kr` 계산기 링크를 넣는다.
   - 태그는 파일 맨 아래 `<!-- 해시태그 ... -->` 주석으로 분리한다.
   - 문체는 검색 유입형 수익 블로그에 맞춘 정보글로 작성한다.
   - 사이트용 글과 같은 문장·문단 순서를 재사용하지 않는다.
   - 제목은 사이트용과 다른 롱테일로 작성하되, 클릭 의도와 검색 키워드를 함께 반영한다.
   - 도입부는 문제 제기와 숫자 후킹을 강화한다.
   - 티스토리용 썸네일은 `1080x1080` WebP 정사각형으로 별도 생성한다.
   - 티스토리용 썸네일은 강한 후킹, 큰 숫자, 클릭 유도형 문구를 사용한다.
   - 썸네일 안에 `티스토리용`, `네이버용` 같은 작업용 라벨을 절대 넣지 않는다.
4. **티스토리용 포스팅 임시저장**
   - `python3 build/tistory_prepare.py blog_post/<파일명>_티스토리.txt`를 실행해 업로드 패키지를 만든다.
   - `tistory_upload/<글이름>/upload-helper.html` 기준으로 제목, 본문, 태그, 대표 이미지를 준비한다.
   - 사용자가 티스토리에 로그인한 Chrome/브라우저 화면을 제공하면 글쓰기 화면에 입력하고 임시저장까지 진행한다.
   - 최종 발행은 사용자 확인 후 진행한다.
   - 사용자가 티스토리 자동화를 원하지 않는다고 하면 브라우저 자동 입력은 하지 않고, 복붙용 텍스트와 대표 이미지만 제공한다.

5. **네이버 블로그용 포스팅 작성**
   - `blog_post/*_네이버.txt` 파일을 별도로 작성한다.
   - 네이버 블로그에 바로 복사할 수 있도록 HTML 태그 없이 자연문 중심으로 작성한다.
   - 제목 후보 3개, 본문, 출처 참고, 네이버 태그를 포함한다.
   - 문체는 생활형 질문·경험형 도입·구어체 중심으로 작성하되, 독자에게 옆에서 설명하듯 따뜻하고 친절한 느낌을 유지한다.
   - 세법 용어를 바로 던지지 말고 일상적인 표현으로 먼저 설명한 뒤 필요한 공식 용어를 덧붙인다.
   - `~해야 합니다` 같은 명령조와 딱딱한 단정을 반복하지 않는다. `~로 보면 쉬워요`, `이 부분만 먼저 확인해보세요`처럼 부담 없이 안내한다.
   - 모바일 가독성을 위해 한 문단은 2~4문장으로 짧게 나누고, 문단 사이에 충분한 빈 줄을 둔다.
   - 핵심 숫자·결론은 긴 문단 안에 묻지 말고 짧은 소제목, 한 줄 요약, 단계형 목록으로 분리한다.
   - 같은 형태의 짧은 문장을 지나치게 연속해서 나열하지 않고, 설명과 예시가 자연스럽게 이어지도록 문장 리듬을 조절한다.
   - 도입부에서 독자의 실제 고민에 공감하고, 본문 중간에는 쉬운 숫자 예시, 마지막에는 바로 실행할 수 있는 확인 순서를 제시한다.
   - 티스토리용 글과 같은 문장·문단 순서를 재사용하지 않는다.
   - 외부 링크는 광고처럼 보이지 않도록 본문 하단에 자연스럽게 1회 정도 배치한다.
   - `[네이버 태그]`에는 해시태그를 반드시 20개 이상 30개 이하로 작성한다.
   - 네이버 태그는 핵심 세금 키워드, 세부 롱테일 키워드, 독자의 생활·직업·상황 키워드를 균형 있게 섞는다.
   - 같은 의미의 표현을 반복하거나 글과 관련 없는 인기 태그로 개수만 채우지 않는다.
   - 네이버용 썸네일은 링크 카드 이미지 영역에 맞춘 `1200x720` WebP(5:3 가로형)로 별도 생성한다.
   - 제목과 핵심 피사체는 중앙 90% 안전영역 안에 배치해 네이버 카드의 축소·크롭에도 잘리지 않게 한다.
   - 네이버용 썸네일은 생활형 질문, 상황 공감, 자연스러운 블로그 피드형 콘셉트로 만든다.
   - 티스토리용 썸네일과 동일한 디자인을 재사용하지 않는다.

### 플랫폼별 산출물 차별화 기준

| 플랫폼 | 제목/롱테일 | 글쓴이 톤 | 구성 | 썸네일 |
|------|-------------|----------|------|--------|
| 세금계산기 사이트 | 정석 SEO, 계산·기준·예시 중심 | 객관적 설명자 | 공식, 계산 예시, FAQ, CTA | 1200x630, 신뢰형 정보 이미지 |
| 티스토리 | 검색 키워드 + 클릭 후킹 | 수익형 정보 블로그 | 문제 제기, 숫자 예시, 광고 블록, CTA | 1080x1080, 강한 후킹/큰 숫자 |
| 네이버 | 생활형 질문·상황형 롱테일 | 친절하게 말을 거는 개인 블로그형 설명자 | 공감 도입, 짧은 문단, 쉬운 용어 풀이, 단계형 요약, 자연스러운 링크 | 1200x720, 5:3 생활형 카드 이미지 |

플랫폼 간 중복을 점검할 때는 아래 기준을 적용한다.

- H1/제목 후보가 서로 달라야 한다.
- 첫 3문단의 전개 방식이 서로 달라야 한다.
- 예시 숫자는 같아도 서술 방식과 배치가 달라야 한다.
- FAQ/질문형 섹션은 플랫폼별 독자 의도에 맞게 바꾼다.
- 썸네일 문구와 시각 콘셉트가 서로 달라야 한다.
- 작업용 라벨이나 내부 용어를 썸네일에 넣지 않는다.

### 티스토리 애드센스 광고 블록

티스토리 원고에는 아래 광고 블록을 기본으로 사용한다. 사이트 본문에는 애드센스 심사 상태를
고려하되, 티스토리는 사용자가 허용했으므로 원고에 광고 코드를 포함한다.

```html
<center><ins class="adsbygoogle" style="display: block;" data-ad-client="ca-pub-2792604427181547" data-ad-slot="4883327520" data-ad-format="auto" data-full-width-responsive="true"></ins>
<script>     (adsbygoogle = window.adsbygoogle || []).push({});</script>
</center>
```

---

## 계산기 모듈 템플릿

```js
const CalcExample = (() => {
  function calculate() {
    // 세금 계산 로직
  }

  function render(result) {
    // DOM 업데이트
  }

  function init() {
    const btn = document.getElementById('btn-example-calc');
    if (!btn) return;
    btn.addEventListener('click', () => {
      const result = calculate();
      render(result);
    });
  }

  return { init };
})();
```

---

## 라우팅 규칙

- URL 해시 형식: `#<category>/<item>` (예: `#real-estate/acquisition`)
- 카테고리 ID: `loan` / `real-estate` / `vehicle` / `income` / `stocks` / `other`
- 뷰 요소 ID: `view-<category>-<item>` (슬래시 → 하이픈, 예: `view-real-estate-acquisition`)

---

## 세율 기준년도

**모든 세율은 2026년 기준**입니다. 세율 변경 시 해당 계산기 파일의 상수만 수정하면 됩니다.

---

## 광고 슬롯 구조

| 슬롯 | 위치 | 크기 | 비고 |
|------|------|------|------|
| 좌측 사이드 | 사이드바 하단 sticky | 160×600 | 1440px+ 표시 |
| 우측 사이드 | 메인 우측 sticky | 160×600 | 1440px+ 표시 |
| 배너 | 탭바와 계산기 패널 사이 | 728×90 | 데스크톱 |
| 앵커 | 화면 하단 고정 | 320×50 | 모바일 |
| 전면 | 외부링크 복귀 시 오버레이 | — | Page Visibility API |

---

## 반응형 브레이크포인트

| 범위 | 설명 |
|------|------|
| < 640px | 모바일 — 햄버거 메뉴, 하단 앵커광고 |
| 640–1023px | 태블릿 |
| 1024px+ | 데스크톱 — 사이드바 고정 표시 |
| 1440px+ | 사이드 광고 표시 |

---

## 주의사항

- `position: fixed`는 카카오/라인 인앱 브라우저에서 버그 발생 → `position: sticky` 사용
- CSS 변수는 `variables.css`에서만 정의, 다크모드는 `[data-theme="dark"]` 셀렉터로 override
- 계산 결과에 사용되는 숫자는 반드시 `UI.fmtWon()` 또는 `UI.fmtNum()`으로 포맷팅
